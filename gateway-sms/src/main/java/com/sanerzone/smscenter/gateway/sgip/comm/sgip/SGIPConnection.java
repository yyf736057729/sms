...
import com.sanerzone.smscenter.gateway.sgip.comm.PReader;
...
import com.sanerzone.smscenter.gateway.sgip.comm.sgip.message.SGIPUnbindMessage;
import com.sanerzone.smscenter.gateway.sgip.util.Args;
import com.sanerzone.smscenter.gateway.sgip.util.Resource;
...
public class SGIPConnection extends SGIPSocketConnection {
	...
	public int getChildId(PMessage message) {
		...
		if (!asClient)
			;
		if (mes.getCommandId() == 4 || mes.getCommandId() == 5 || mes.getCommandId() == 17 || mes.getCommandId() == 1
				|| mes.getCommandId() == 2)
			return -1;
		else
			return sequenceId;
	...
	protected synchronized void connect() {
		...
		if (rsp != null && rsp.getResult() != 0) {
			...
			if (rsp.getResult() == 1)
				...
				setError(SGIPConstant.OTHER_ERROR + "-sgip:" + rsp.getResult());
		}
