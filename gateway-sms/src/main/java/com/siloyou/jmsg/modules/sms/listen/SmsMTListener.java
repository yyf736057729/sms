...
import com.siloyou.jmsg.common.util.MessageExtUtil;
...
public class SmsMTListener implements MessageListener {
    private Logger logger = LoggerFactory.getLogger(SmsMTListener.class);
    ...
    public Action consume(Message msg, ConsumeContext consumeContext) {
        ...
        try
        {
            ...
            logger.info("storage=> topic:{}, tags:{}, msgid:{}, key:{}",
                    msg.getTopic(), msg.getTag(), msg.getMsgID(), msg.getKey());
...
//            smsMtMessage = MessageExtUtil.convertMessageExt(SmsMtMessage.class, msg);
            ...
            if (null != smsMtMessage)
            {
                ...
                jmsgSmsSend.setTopic(msg.getTag());//发送队列
