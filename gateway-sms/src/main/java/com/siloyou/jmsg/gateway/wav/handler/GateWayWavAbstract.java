package com.siloyou.jmsg.gateway.wav.handler;
...
public abstract class GateWayWavAbstract
{
    ...
    public GateWayWavAbstract(String gateWayID, String jsonParams)
    {
    ...
    public SmsSrMessage initGatewayResultMessage(String httpBody, SmsMtMessage message, String key)
    {
        ...
        Map<String, String> map = JSON.parseObject(httpBody, Map.class);
        if (StringUtils.equals(map.get("status"), "4001") || StringUtils.equals(map.get("status"), "FAIL"))
        {
            srMessage.setResult("F10102" + StringUtils.substring(httpBody, 16));
            srMessage.setReserve("接口调用异常");
        }
    ...
    public SmsRtMessage initSmsRtMessage(String msgid, String destTermID, String stat, String successStat){
    	...
    	if( null != smsMtMessage) {
    		...
    		smsRtMessage.setSmscSequence(smsMtMessage.getId());
     ...
     * @see [类、类#方法、类#成员]
     */
    public abstract boolean buildTTS(SmsMtMessage mt);
