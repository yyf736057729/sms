package com.siloyou.jmsg.gateway.http;
...
public class HttpGatewayFactory extends BaseGatewayFactory 
{
    ...
    private synchronized boolean initGate(JmsgGateWayInfo gateway)
    {
        ...
        if (GateEnum.HTTP.getValue().equalsIgnoreCase(gateway.getType()))
        {
            ...
            try
            {
                ...
                GateWayHttpAbstract gatewayImpl = constructor.newInstance(gateWayID, gateway.getExtParam());
                GATEWAY_MAP.put(gateWayID, gatewayImpl);
                
                synchronized (this) {
					SmsGateWay smsGateWay = getSmsGateWay(gateway.getId());
					if(smsGateWay == null) {
						smsGateWay = new SmsGateWay(gateway.getId(), gateway.getWriteLimit(), (CMPPGatewayFactory) this);
					}
					smsGateWay.startup();
				}
    ...
    public Object getGateway(String gateWayID)
    {
        ...
        return (GateWayHttpAbstract)GATEWAY_MAP.get(gateWayID);
    }
