...
@RestController
@RequestMapping(value = "/api/wav/report")
public class WavGatewayReport {

    private final static Logger   logger  = Logger.getLogger(WavGatewayReport.class);
    ...
    public String push(String code, String respText, HttpServletRequest request, HttpServletResponse response){
        ...
        if (StringUtils.isNotBlank(respText)) {
            GateWayWavAbstract executer = (GateWayWavAbstract)getGatewayFactory().getGateway(code);
            if( executer != null) 
            {
