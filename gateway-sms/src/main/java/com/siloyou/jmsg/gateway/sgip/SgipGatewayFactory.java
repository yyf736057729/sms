...
//        gatewayService.updateGatewayStateById(GateStateEnum.DISABLED.getValue(), id);
        com.sanerzone.smscenter.gateway.sgip.SGIPSMProxy proxy = (com.sanerzone.smscenter.gateway.sgip.SGIPSMProxy) GATEWAY_MAP.remove(id);
        try
        {
            //关闭队列
...
//    	}
    	...
    	com.sanerzone.smscenter.gateway.sgip.SGIPSMProxy proxy = (com.sanerzone.smscenter.gateway.sgip.SGIPSMProxy)this.getGateway(msg.getGateWayID());
        if (proxy == null)
        {
...
//    		Submit submit = new Submit(frame.getTppid(), frame.getTpudhi(), SPNumber, ChargeNumber, UserNumber, CorpId,
//    				nodeid, serviceType, 1, 0, 0, 1,
//    				3, 0, null, null,
//    				1, frame.getMsgfmt().getValue(), 0, frame.getMsgContentBytes(),
//    				frame.getMsgLength(), "0");
    ...
    private void runMonitorTask() {
        ...
        while (gatewayIt.hasNext()) {
            ...
            buff.append("网关ID：" + gEntry.getKey() + ",网关状态:" + ((com.sanerzone.smscenter.gateway.sgip.SGIPSMProxy)gEntry.getValue()).getStatus() + ";\r\n");
        }
