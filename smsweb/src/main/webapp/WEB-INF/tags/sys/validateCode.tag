...
<%-- <input type="text" id="${name}" name="${name}" maxlength="5" class="txt required" style="color: black;font-weight:bold;width:65px;${inputCssStyle}"/> --%>
<input type="text" id="${name}" name="${name}" maxlength="5" class="form-control form-yzm required" placeholder="验证码">
...
<p><a href="javascript:" onclick="$('.${name}').attr('src','${pageContext.request.contextPath}/servlet/validateCodeServlet?'+new Date().getTime());" class="mid ${name}Refresh" style="${buttonCssStyle}">换一张</a></p>