...
<html>
<head>
	<title>用户的平台统计月账单</title>
	<meta name="decorator" content="default"/>
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnExport").click(function(){
				top.$.jBox.confirm("确认要导出用户的平台统计月账单数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/sms/jmsgSmsDayReport/exportByMonth");
						$("#searchForm").submit();
					}
				},{buttonsFocus:1});
		...
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/sms/jmsgSmsDayReport/listByMonth");
			$("#searchForm").submit();
        	return false;
        }	
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/sms/jmsgSmsDayReport/listByMonth">信息列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="jmsgSmsDayReport" action="${ctx}/sms/jmsgSmsDayReport/listByMonth" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="jmsgSmsDayReport">
			<tr>
				<td>
					<a href='javascript:viewByPhoneType("${jmsgSmsDayReport.user.id}","<fmt:formatDate value="${jmsgSmsDayReport.day}" pattern="yyyy-MM-dd"/>","month","${jmsgSmsDayReport.user.name}")'>详情</a>
				</td>
				<td>
