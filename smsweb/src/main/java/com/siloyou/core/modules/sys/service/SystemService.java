...
import com.siloyou.core.common.security.Digests;
import com.siloyou.core.common.security.shiro.session.SessionDAO;
import com.siloyou.core.common.service.BaseService;
import com.siloyou.core.common.service.ServiceException;
import com.siloyou.core.common.utils.CacheUtils;
import com.siloyou.core.common.utils.Encodes;
import com.siloyou.core.common.utils.StringUtils;
import com.siloyou.core.modules.sys.dao.MenuDao;
...
import com.siloyou.core.modules.sys.security.SystemAuthorizingRealm;
import com.siloyou.core.modules.sys.utils.LogUtils;
...
public class SystemService extends BaseService implements InitializingBean {
	...
	@DubboReference
	private AccountConfigInterface accountConfig;
...
//		user.getSqlMap().put("dsf", dataScopeFilter(user.getCurrentUser(), "o", "a"));
		user.getSqlMap().put("dsf", dataScopeFilter(user.getCurrentUser(), "c", "a"));
		// 设置分页参数
		user.setPage(page);
		// 执行分页查询
...
//		user.getSqlMap().put("dsf", dataScopeFilter(user.getCurrentUser(), "o", "a"));
	...
	public List<User> findUserByOfficeId(String companyId) {
		List<User> list = (List<User>)CacheUtils.get(UserUtils.USER_CACHE, UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + companyId);
		if (list == null){
			User user = new User();
			user.setCompany(new Office(companyId));
			list = userDao.findUserByOfficeId(user);
			CacheUtils.put(UserUtils.USER_CACHE, UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + companyId, list);
		}
		return list;
	}
	...
	public void saveUser(User user) {
		if (StringUtils.isBlank(user.getId())){
			user.preInsert();
			logger.info("UserDao.xml===>>>insert");
			userDao.insert(user);
			
			//开通短信信账号
			jmsgAccountService.save(user.getId(), "sms", UserUtils.getUser().getId(),"2");
			String key = AccountCacheUtils.getAmountKey("sms", user.getId());
//			JedisClusterUtils.set(key, "0", 0);
		}else{
			// 清除原用户机构用户缓存
			User oldUser = userDao.get(user.getId());
			if (oldUser.getCompany() != null && oldUser.getCompany().getId() != null){
				CacheUtils.remove(UserUtils.USER_CACHE, UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + oldUser.getCompany().getId());
			}
			// 更新用户数据
			user.preUpdate();
			userDao.update(user);
		...
		if (StringUtils.isNotBlank(user.getId())){
			...
			if (user.getRoleList() != null && user.getRoleList().size() > 0){
				for(Role role : user.getRoleList()){
					...
					userDao.insertUserRoleFix(user1);
				}
...
//			systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void saveUser(User user,String payMode,String appType) {
		if (StringUtils.isBlank(user.getId())){
			user.preInsert();
			userDao.insert(user);
			
			//开通短信账号
			jmsgAccountService.save(user.getId(), appType, UserUtils.getUser().getId(),payMode);
			...
			user.preUpdate();
			userDao.update(user);
			jmsgAccountService.updatePayMode(user.getId(), appType, payMode);
		}
		if (StringUtils.isNotBlank(user.getId())){
			// 更新用户与角色关联
			userDao.deleteUserRole(user);
...
//			systemRealm.clearAllCachedAuthorizationInfo();
     ...
     */
	@Transactional(readOnly = false)
    public void batchUpdateUserGroup(JmsgAccount jmsgAccount)
    {
	    User oldUser = null;
	    ...
	    if (null != userList)
	    {
	        ...
	        {
	            oldUser = UserUtils.get(tmpUser.getId());
	            oldUser.setGroupId(jmsgAccount.getUser().getGroupId());
	            
	            UserUtils.putCache(tmpUser.getId(), oldUser);
	        }
	    }
	    Map<String, Object> map = new HashMap<String, Object>();
	    map.put("groupId", jmsgAccount.getUser().getGroupId());
	    map.put("companyId", jmsgAccount.getUser().getCompany().getId());
	    
	    userDao.batchUpdateUserGroup(map);
    }
     ...
     */
	@Transactional(readOnly = false)
    public void updateUserGroupById(JmsgAccount jmsgAccount)
    {
        User user = UserUtils.get(jmsgAccount.getUser().getId());
        user.setGroupId(jmsgAccount.getUser().getGroupId());
        UserUtils.putCache(jmsgAccount.getUser().getId(), user);
        
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("groupId", jmsgAccount.getUser().getGroupId());
        map.put("id", jmsgAccount.getUser().getId());
        userDao.updateUserGroupById(map);
        dubboCache(user.getId());
    }
    ...
    {
        User user = userDao.getAgency(companyId);
        
        return user;
    }
	
	@Transactional(readOnly = false)
	public void saveUserByAgency(User user,String appType,String payMode,String userType) {
		if (StringUtils.isBlank(user.getId())){
			user.preInsert();
			user.setUserType(userType);//普通用户
			userDao.insert(user);
			
			//appType 账号类型 sms:短信,mms:彩信
			jmsgAccountService.save(user.getId(), appType, UserUtils.getUser().getId(),payMode);
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public List<Role> findAllRole(){
		return UserUtils.getRoleListNew();
	}
	...
	public void saveRole(Role role) {
		...
		if(StringUtils.isBlank(role.getSysData()))role.setSysData("0");
		role.setUseable("1");
		role.setDataScope("2");
...
//		}
	...
	//同步用户信息
	public void dubboCache(String userId){
		Map<String,String> result = Maps.newHashMap();
		JmsgAccount jmsgAccount = new JmsgAccount();
		jmsgAccount.setUserId(userId);
		jmsgAccount.setAppType("sms");
		List<Map<String,String>> list = jmsgAccountService.getMapByUser(jmsgAccount);
		if(list != null && list.size() == 1){
			for (Map<String, String> map : list) {
				result.putAll(map);
			}
			accountConfig.configAccount(1, userId, result);
		}
	}
