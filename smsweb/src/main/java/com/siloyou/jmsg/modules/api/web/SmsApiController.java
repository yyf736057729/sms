...
//				sign = AccountCacheUtils.getStringValue(userId, "forceSign", "");	//强制签名
            ...
            if (count > 0)
            {
                ...
                {//自动审核
                    ...
                    else
                    {
                        ...
                        reviewStatus = dataMap.get("reviewStatus");//审核标识 8:免审 9:待审
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    @RequestMapping(value = "sms/sendbatch")
    public String smsSendBatch(HttpServletRequest request, HttpServletResponse response){
    	logger.info("sms api batch: {}", JsonMapper.toJsonString(request.getParameterMap()));
        Map map = Maps.newHashMap();
        try{
            String userId = request.getParameter("userid");
            String sendtime = request.getParameter("time");
            ...
            if (phones.length() > 600000){//单次最多 50000个手机号码 TODO
                map.put("code", "14");
                map.put("msg", "号码个数超过限制");
                return result(response, map);
            }
            ...
            if (count > 0){
        		...
        		**/
                ...
                JmsgSmsTask jmsgSmsTask= new JmsgSmsTask();
                String taskId = new MsgId(1000002).toString();
                jmsgSmsTask.setSendDatetime(sendDatetime != null ? sendDatetime : new Date());
        		jmsgSmsTask.setId(taskId);
        		jmsgSmsTask.setSmsContent(smsContent);
        		jmsgSmsTask.setSendCount(count);
        		jmsgSmsTask.setCreateBy(UserUtils.get(userId));
        		jmsgSmsTask.setTaskType("3");//批量提交
        		
        		if ("0".equals(nocheck)){//必审
        			jmsgSmsTask.setStatus("-1");
        		}else{
        			jmsgSmsTask.setStatus("1");
        		}
        		
                String res = jmsgSmsTaskService.createSmsTask(jmsgSmsTask, phoneList, payCount);
                if ("1".equals(res)){
                	 map.put("code", "1");
                     map.put("msg", "提交失败");
                }else{
...
//        Map<String,String> sendMap = jmsgSmsTaskService.createSendDetail(dataId, taskId, userId, phoneList, smsContent, "API", mq,sign,payCount);
...
//    }
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
	@RequestMapping(value = "sms/mmsSend")
    public String mmsSend(HttpServletRequest request, HttpServletResponse response)
    {
    	logger.info("mms api v2: {}", JsonMapper.toJsonString(request.getParameterMap()));
        ...
        try
        {
            ...
            List<String> phoneList = Arrays.asList(phones.split(","));//获取号码
            ...
            if (count > 0)
            {
                ...
                if ("0".equals(nocheck) || phoneList.size() > 50 || sendDatetime != null)
                {
                    ...
                    String res = jmsgSmsTaskService.createSmsTask(jmsgSmsTask, phoneList, 0, payCount);
                    if ("短信接收号码导入失败".equals(res))
                    {
                    	 map.put("code", "1");
                         map.put("msg", "提交失败");
                ...
                else
                {
                    ...
                    message.setSendTime(SystemClock.now());
                    message.setCstmOrderID(request.getParameter("messageid"));
                    message.setContentSize(1);//彩信大小
                    message.setWapUrl("彩信地址");//短信地址
