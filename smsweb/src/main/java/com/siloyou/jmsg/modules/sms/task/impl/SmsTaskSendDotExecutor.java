...
public class SmsTaskSendDotExecutor implements Runnable {
	...
	public SmsTaskSendDotExecutor(String taskid,int version,int rowNumber) {
		this.taskid = taskid;
			...
			while(true) {
				...
				try{
					...
					for(String line = br.readLine();line !=null;line=br.readLine()){
						...
						smsContent = array[1];
						sign = SignUtils.get(smsContent);
						payCount = jmsgSmsTaskService.findPayCount(smsContent);
						smsType = payCount > 1 ?"2":"1";
						if(com.sanerzone.common.support.utils.JedisClusterUtils.decrBy(key, payCount) < 0){//余额不足
							com.sanerzone.common.support.utils.JedisClusterUtils.incrBy(key, payCount);
						...
						JmsgSmsSend jmsgSmsSend = jmsgSmsTaskService.createSendDetailNew(user, tableName, taskid, array[0], smsContent, sign, payCount, payType, pushFlag, smsType, true);
						...
						logger.info("TASKID:{},点对点短信发送提交队列情况:{}",  taskid, map);
					}
					sqlSession.commit();
				}catch(Exception e){
