package com.siloyou.jmsg.modules.swsms.task.impl;
...
import com.siloyou.jmsg.modules.swsms.service.SwSmsSendService;
import com.siloyou.jmsg.modules.swsms.service.SwSmsTaskService;
import com.siloyou.jmsg.modules.swsms.task.SwSmsSendTask;

public class SwSmsTaskSendExecutor implements Runnable {

	public static Logger logger = LoggerFactory.getLogger(SwSmsTaskSendExecutor.class);
	private static JmsgSmsTaskDao taskDao = SpringContextHolder.getBean(JmsgSmsTaskDao.class);
	private static SwSmsTaskService swSmsTaskService = SpringContextHolder.getBean(SwSmsTaskService.class);
	...
	SwSmsSendService swSmsSendService = SpringContextHolder.getBean(SwSmsSendService.class);
	...
	public SwSmsTaskSendExecutor(String taskid,int version,int rowNumber) {
		this.taskid = taskid;
	...
	public void run() {
		...
		try {
			while(true) {
				...
				//获取待发送数据
				//File file = new File(Global.getConfig("swSmsTask.phoneList.dir")+File.separator+taskid+".txt");
				...
				try {
					...
					for(String line = br.readLine();line !=null;line=br.readLine()){
						...
						//发送 TODO
						JmsgSmsSend jmsgSmsSend = swSmsTaskService.createSendDetailNew(user, tableName, taskid, line, smsContent, sign, payCount, payType, pushFlag, smsType, false);
				...
				SwSmsSendTask.smsTaskSendExecMap.remove(taskid);// 任务执行完成
				break;
			}
		} catch (Exception e) {
			logger.error("批量任务发送失败",e);
	...
	private Map<String,String> taskRunResult(Map<String,String> map,int index){
		...
		if(task != null){
			...
			if ("5".equals(sendStatus)||"9".equals(sendStatus)) {// 任务暂停 或任务停止
				...
				SwSmsSendTask.smsTaskSendExecMap.remove(taskid);// 剔除线程
				runFlag = "0";
			}
...
*/				Map<String,String> sendMap=swSmsSendService.sendSms(entity);
