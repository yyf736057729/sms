package com.siloyou.jmsg.modules.swsms.task;
...
import com.siloyou.jmsg.modules.swsms.task.impl.SwSmsTaskSendExecutor;

//定时任务
//@Service
//@Lazy(false)
public class SwSmsSendTask {
	
	public static Map<String, SwSmsTaskSendExecutor> smsTaskSendExecMap = Maps.newConcurrentMap(); 
		...
		if(poolTaskSize <20){
			...
			if(list != null && list.size() >0){
				for (JmsgSmsTask jmsgSmsTask : list) {
					...
					SwSmsTaskSendExecutor send = new SwSmsTaskSendExecutor(taskid,jmsgSmsTask.getVersion(),jmsgSmsTask.getRowNumber());
					synchronized (send) {
